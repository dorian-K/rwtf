...
class TransformerDecoder(torch.nn.Module):
    def __init__(self, d_model=64, nhead=4, num_layers=2, dropout_rate=0.1):
        super(TransformerDecoder, self).__init__()
        
        self.positional_encoding = nn.Embedding(216, d_model)
        # initialize positional encoding with sinusoidal values
        position = torch.arange(0, 216, dtype=torch.long).unsqueeze(1)  # (seq_len, 1)
        div_term = torch.exp(torch.arange(0, d_model, 2).float() * -(torch.log(torch.tensor(512.0)) / d_model))
        pe = torch.zeros(216, d_model)
        pe[:, 0::2] = torch.sin(position * div_term)  # even indices
        pe[:, 1::2] = torch.cos(position * div_term)  # odd indices
        self.positional_encoding.weight = nn.Parameter(pe, requires_grad=True)  # freeze positional encoding
    ...
    def forward(self, x: torch.Tensor):
        ...
        if self.training:
            ...
            x = x + noise_additive
            x = torch.clamp(x, 0, 250)  # ensure values are between 0 and 250
        else:
        ...
        x = torch.log(x + 1) / torch.log(torch.tensor(251.0))
        ...
        x = torch.exp(x * torch.log(torch.tensor(251.0))) - 1  # scale back to original value range
